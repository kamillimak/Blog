...
## 16. INSTRUKCJA DLA TRAE: PRZEBUDOWA STRONY GŁÓWNEJ POD NEWSY DZIENNE

Trae ma przebudować layout strony głównej tak, aby newsy były prezentowane z podziałem na dni oraz czytelną datą publikacji. Nie przepisuj newsów ręcznie do komponentów, jeżeli istnieją już w plikach projektowych.

### Źródła danych, z których Trae ma korzystać
- `content/daily-news/YYYY-MM-DD/YYYY-MM-DD.md` — archiwum briefingów dziennych; ma służyć jako źródło podziału na dni i dat publikacji.
- `content/draft/YYYY-MM-DD-daily-news/YYYY-MM-DD.md` — redakcyjne kopie szkiców importowane do aplikacji jako `DRAFT`.
- `src/data/draftArticles.ts` — obecny mechanizm importu Markdown przez `import.meta.glob`; przy przebudowie używaj istniejącego importu lub wydziel z niego osobny, typowany helper zamiast dublować parser.
- `src/components/article/DailyBriefing.tsx` — obecnie pokazuje jeden ręcznie wpisany dzień; docelowo powinien stać się komponentem osi czasu / dziennego feedu.
- `src/pages/HomePage.tsx` — miejsce integracji nowego layoutu.

### Wymagania funkcjonalne
- Strona główna ma pokazywać newsy pogrupowane po dniach, np. `3 lipca 2026`, `4 lipca 2026`, `5 lipca 2026` itd.
- Każda grupa dnia ma pokazywać datę publikacji, status (`DRAFT`/`APPROVED`, jeśli dostępny), metrykę podziału geograficznego `Polska X / Świat Y` oraz listę 5 newsów z kategorią, tytułem, krótkim opisem i linkiem do źródła.
- Najnowszy dzień powinien być najwyżej; starsze dni niżej w osi czasu.
- Użytkownik powinien móc szybko filtrować lub przełączać widok: `Wszystkie`, `Polska`, `Świat`.
- Zachowaj linki do pełnych szkiców artykułów importowanych z `content/draft`, ale nie mieszaj ich z archiwum `content/top-3` jako głównym źródłem dziennego briefingu.

### Wymagania UI
- Layout ma być magazynowy, premium i skanowalny: sekcje pełnej szerokości, bez zagnieżdżania kart w kartach.
- Dzień publikacji ma być pierwszoplanowym elementem każdej grupy, nie małym tekstem pomocniczym.
- Na desktopie preferuj układ osi czasu lub dwukolumnowy feed: lista dni po lewej / szczegóły dnia po prawej albo pełne sekcje dzień po dniu.
- Na mobile grupy dni mają układać się pionowo, bez poziomego scrolla i bez nachodzącego tekstu.
- Używaj `lucide-react` dla ikon i Tailwind v4 zgodnie z obecnym stylem projektu.
- Nie używaj `dangerouslySetInnerHTML`; Markdown musi być sparsowany do strukturalnych danych albo mapowany przez istniejące typy.

### Granice techniczne
- Nie zmieniaj statusu żadnego materiału na `APPROVED`.
- Nie hotlinkuj grafik zewnętrznych; brakujące media pozostają placeholderami.
- Nie konwertuj plików Markdown do Google Docs.
- Po zmianach uruchom `npm run lint` i `npm run build`.
- Jeżeli zmieniasz parser Markdown, zachowaj kompatybilność z istniejącymi szkicami i krótkimi slugami `#/articles/MMRR/DD`.
